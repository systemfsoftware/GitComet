...
# daemon would cold-index the whole tree. The new branch shares the primary's
# code, so the primary's index is a valid warm start.
#
# NOTE (probe 2026-08-16): this repo has no .codegraph/ — the wt.toml omits the
# codegraph hook line entirely, so this script never runs. It ships as part of
# the portable set (self-guarding) for the day a codegraph index exists.
#
